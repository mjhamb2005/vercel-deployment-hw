...
export default function CaptionsList() {
  ...
  async function submitVote(captionId, voteValue) {
    ...
    if (cur === voteValue) {
      ...
      const { error } = await supabase.from('caption_votes').update({ vote_value: voteValue, modified_by_user_id: user.id }).eq('caption_id', captionId).eq('profile_id', user.id)
      if (!error) setVotes(p => ({ ...p, [captionId]: voteValue }))
    } else {
      const { error } = await supabase.from('caption_votes').insert({ caption_id: captionId, profile_id: user.id, vote_value: voteValue, created_by_user_id: user.id, modified_by_user_id: user.id })
      if (!error) setVotes(p => ({ ...p, [captionId]: voteValue }))
    }
    setSubmitting(null)
  }
